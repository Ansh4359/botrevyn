...
def security_node(state: ReviewState) -> dict:
    ...
    if active and "security" not in active:
        return {}
    ...
    system_prompt = (
        "You are an expert Security Engineer looking for vulnerabilities such as injection, XSS, SSRF, "
        "hardcoded secrets, insecure dependencies, and auth issues. Analyze the provided pull request changes. "
        "Pay special attention to input validation, SQL queries, file operations, network calls, crypto usage, and secret patterns. "
        ...
        "'category' (SECURITY), 'auto_fixable' (boolean)."
    )
    ...
    try:
        ...
        return {"errors": [f"security_node error: {str(e)}"]}
